...
class TariffCalculateUseCaseTest {
    ...
    void whenCalculatePrice_thenSuccess() {
        ...
        var cubeMeterCost = new Price(BigDecimal.valueOf(3500), currency);

        when(weightPriceProvider.minimalPrice()).thenReturn(minimalPrice);
        when(weightPriceProvider.costPerKg()).thenReturn(pricePerKg);
        when(volumePriceProvider.cubeMeterCost()).thenReturn(cubeMeterCost);

        var shipment = new Shipment(List.of(new Pack(
                new Weight(BigInteger.valueOf(1200)),
                new Height(BigInteger.valueOf(500)),
                new Length(BigInteger.valueOf(300)),
                new Width(BigInteger.valueOf(400)))),
                new CurrencyFactory(code -> true).create("RUB"));
        var expectedPrice = new Price(BigDecimal.valueOf(210.00), currency);
